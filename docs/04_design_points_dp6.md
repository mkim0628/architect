...
> **[구 체계 동결 — 2단계 이관]** 근접 연산 오프로드는 배경 v5에서 2단계로
> 이관되었다. 1단계 현행 DP 체계는
...
> throughput(decode 축) 각 ≥ 2×**로 개정됨에 따라 QA1 행을 재채점. DP6은
> **throughput 축**(decode 연산 오프로드)에 작용하므로 그 축의 배율을 신
> bin으로 판정하고 TTFT 축은 재사용·retrieval DP(DP3·DP7·DP8) 책임임을
> 병기한다 (상세 규칙: 02 문서 §0 주석).
...
순서·정밀도가 GPU와 달라 softmax/accumulation 검증 부담이 생기고(QA2),
...
|----|------|-----------|
| QA1 | ★★☆ (F) | **throughput 축(decode)**: 압축/해제의 GPU 사이클 회수 + 더 공격적 압축 운용으로 간접 개선(C)이나 decode attention(wait 70–85%(A)의 지배분)은 그대로 — throughput 2× 미달(1.5–2×). TTFT와 직교. 종합 ★★☆ |
| QA2 | ★★★ (F) | 수치 경로 불침범 — codec은 결정론적 변환, acc 저하 0%p·ΔPPL 0 (압축 기법 자체의 품질 비용은 DP2·QA2에서 이미 계상)(C) |
| QA3 | ★★★ (F) | ≥3× bin 경로: in-place 압축으로 압축 비용이 하락 → 더 높은 압축률·더 넓은 압축 적용을 감당 — 압축 2–4×(A)의 적용 범위 확대(C) |
...
| QA | 평점 | 정량 근거 (00 v0.3 bin 판정) |
|----|------|-----------|
| QA1 | ★★★ (상한) / ★★☆ (도달 리스크) (F) | **throughput 축(decode)**: attention 오프로드로 decode 지배분 직격 — 최대 2.81×(B: AttAcc), throughput **2× 초과 경로**. 도달 리스크: GQA로 산술 강도가 오르면 PIM 이점 축소, batch 구성·파이프라인 효율에 종속(C). TTFT와 직교 |
| QA2 | ★★☆ (F) | 결정론적 HW 연산이나 누적 순서·정밀도가 GPU와 상이 — acc ≤2%p 전역 bound 운용은 가능(C)하되 요청별 집행 문제 이전에 **커널 수치 검증**이 선행 부담. ★★★ 미달 |
| QA3 | ★★☆ (F) | 1.5–3× bin: 용량 축 직접 기여 없음 — 압축은 여전히 GPU 실행이라 적용 범위 제한(C). tier 승격 트래픽 절감은 있으나 원본 환산 배율엔 간접적 |
...
- 실질 결정 변수는 "**수치 경로 침범이라는 검증 비용의 단절점을 넘어야만
  잡히는 이득이 과제 목표(QA1 throughput 축 2×)에 필수인가**"다. 후보1의
  간접 이득(압축 비용 회수)이 QA1 throughput 축 ★★☆(1.5–2×)에 그친다는
  예측이 맞다면, **decode 축 2× 달성**에는 후보2가 필요하다는 결론이
  실측으로 나올 수 있다 — AttAcc 시뮬레이터(공개)
