...
> throughput(decode 축) 각 ≥ 2×**로 개정됨에 따라 QA1 행을 재채점. DP3은
> **TTFT 축**(재사용), DP4·DP5는 **throughput 축**(tier·압축·flip)에
> 작용하므로 담당 축의 배율을 신 bin으로 판정하고 상보 축은 sibling DP
> 책임임을 병기한다 (상세 규칙: 02 문서 §0 주석).
...
|----|------|-----------|
| QA1 | ★★☆ (F) | **TTFT 축(prefill)**: multiturn·시스템 프롬프트 prefix 히트로 재계산 일부 절감(B: prefix caching) → TTFT 1.5–2× 경로. 비연속 공유 chunk(CacheBlend TTFT 2.2–3.3×(B)) 미회수로 TTFT 2× 미달(C). throughput은 decode 축 DP(DP2·DP4·DP6) 담당 — 본 DP는 prefill 축 책임. 종합 ★★☆ |
...
| QA | 평점 | 정량 근거 (00 v0.3 bin 판정) |
|----|------|-----------|
| QA1 | ★★★ (상한) / ★★☆ (도달 리스크) (F) | **TTFT 축(prefill)**: 비연속 재사용으로 TTFT 2.2–3.3×(B: CacheBlend) — **2× 초과 경로**, prefill 축을 단독으로 목표에 올림(부수 처리율 2.8–5×(B)도 decode 축에 기여). 도달 리스크: hit율 종속 — 공유도 낮은 워크로드에선 1.5–2× 하향(C). throughput 2×는 decode 축 DP가 완성 |
...
| QA | 평점 | 정량 근거 (00 v0.3 bin 판정) |
|----|------|-----------|
| QA1 | ★★☆ (F) | **throughput 축(decode)**: tiering+압축 batch 기본 이득은 수치 축으로 회수(B: KIVI)나 압축/해제 연산이 GPU에 남아 decode 사이클 잠식 — 근접연산 오프로드 미사용으로 throughput 2× 미달(1.5–2×)(C). TTFT와 직교. 종합 ★★☆ |
...
| QA | 평점 | 정량 근거 (00 v0.3 bin 판정) |
|----|------|-----------|
| QA1 | ★★★ (F) | **throughput 축(decode)**: 압축/해제를 PIM/PNM에 오프로드해 decode GPU 사이클 보존 — decode wait 70–85%(A) 구간에 tiering + 오프로드 이득 중첩, throughput **2× 경로**(C, 압축 단독 2.35–3.47×(B) + 오프로드로 후보1 GPU-사이클 감점 회수). TTFT는 재사용 DP 담당 — 본 DP는 decode 축 책임 |
...
| QA | 평점 | 정량 근거 (00 v0.3 bin 판정) |
|----|------|-----------|
| QA1 | ★★☆ (F) | **throughput 축(decode)**: 정상 상태 throughput은 tiering 이득 그대로(2× 경로 가능)이나 부하 급변 시 flip drain(O(KV/BW))이 재배치 지연 — TPOT tail↑, iso-latency 판정에서 1.5–2×(C). TTFT와 직교. 종합 ★★☆ |
...
| QA | 평점 | 정량 근거 (00 v0.3 bin 판정) |
|----|------|-----------|
| QA1 | ★★★ (상한) / ★★☆ (도달 리스크) (F) | **throughput 축(decode)**: drain-free flip으로 부하 추종 재배치가 민첩 — 급변 워크로드에서 iso-latency throughput **2× 경로**(C). 도달 리스크: 공유 tier BW 포화 시 승격 지연이 tail 잠식 — CXL BW 열위(B 일반론)로 fan-in 실측 전까지 1.5–2× 불확실(C). TTFT와 직교 |
