...
| C-03 | 모델 무변경 (training-free) | 목표 2의 "LLM 정확도 유지"는 **모델 재학습·fine-tuning·구조 변경 없이** 달성해야 한다 — 압축·재사용은 런타임 계층의 기법으로 한정하고, 모델 차원의 KV 축소(MLA 등)는 모델 설계 영역으로 범위 외(그 **수용**은 QA4). 품질 판정은 동일 모델의 비압축 구성 대비로만 수행한다(QA2 baseline). | 과제 목표 정의 (R-24, v1.0 신설) |
...
| UC-08 | 메모리 압박 대응 (압축/강등/축출 중 goodput 최적 선택) — UC-05·06을 «include» | FR-05 |
...
Utility Tree에 전량 수록. 출처 VOC 매핑: QA-01 ← R-01·R-11·R-24 (R-07·R-09는
2단계 배정 — 부록 A) / QA-02 ← R-06 / QA-03 ← R-01·R-02 / QA-04 ← R-13·R-17·R-21 /
...
|---|---|---|---|---|---|---|---|
| QA-01 | Performance | prefill·decode 2단 성능 — baseline 대비 **TTFT 단축 배율 · throughput 배율** (목표 1 → TTFT, 목표 2·3 → throughput) | 대표 워크로드(long-context RAG·multiturn·agent)를 동일 HW·동일 실행 구성에서 E2E 서빙하며 두 지표를 잰다: ① **TTFT 단축 배율**(prefill 축 — KV 재사용[목표 1] 반영, 평균 판정·p99 병행) ② **throughput 배율**(decode 축 — 압축·tier 확장의 batch 확대[목표 2] + KV 인지 스케줄링[목표 3] 반영, iso-latency: TPOT p99 ≤ baseline 운영점·곡선 병행). 목표 3축의 순기여는 **축별 ablation**(재사용 off / 압축 off / KV-blind 스케줄링)으로 분리 보고. [측정: **TTFT·throughput 모두 ≥ 2×** → ★★★. baseline = 동일 HW에서 **GPU HBM 단일 tier만 사용**하는 구성 — 순증분 분리 측정. P/D 분리는 전제하지 않는 실험 변수(양쪽 동일 적용)] | H | H | 1 | **O** |
...
| QA1 추론 성능 | 전 DP 평가의 1순위 축 (baseline = 동일 HW·GPU HBM 단일 tier + 축별 ablation) | DP1–DP5 QA 평가표 |
| QA2 응답 품질 (gate) | 요청별 품질 bound의 집행 구조 (전역 아닌 요청 단위) — 성능·용량 수치의 유효 전제 | DP2·DP3 |
| QA3 메모리 효율 | tier 오프로딩 × 압축의 결합 구조 요구 (압축 단독 초과) | DP2·DP4 |
| QA4 확장성·진화성 | 코어/모듈 경계와 공개 인터페이스의 안정성 설계 (KV 구조 변화 + 2단계 디바이스 수용 대비) | DP1·DP4 / KV Locator · CompressionOp |
...
| C-03 모델 무변경 (training-free) | 압축·재사용 기법 선택지를 런타임 계층으로 한정 — 품질 회복을 재학습에 기댈 수 없어 요청별 bound 집행 구조(QA2)의 중요성이 커짐 | DP2·DP3 전제 / QA2 |
...
| R-02 | MCAS 팀 | "우리가 시뮬레이션으로 예측한 이종 메모리 시스템의 효과가 실제 SW 스택 실측에서도 재현되는지 — 같은 HBM으로 동시 컨텍스트를 얼마나 더 수용하는지 정량으로 보고 싶다." | QA-03, FR-03, FR-04 |
...
| R-11 | 개발 임원 | "GPU HBM 단일 tier baseline 대비 개선을 E2E 정량으로 입증해야 사업 설득이 된다. 대표 워크로드 벤치가 완주해야 한다." | QA-01(baseline), FR-01, FR-06 |
...
| R-17 | MCR 개발팀 | "tier 조합·P/D 구성을 바꿔가며 실험한다 — 신규 tier의 등록이 쉬워야 하고, 고정된 노드들 안에서 prefill/decode role 전환·재배분이 자동이어야 구성 변경 실험을 반복할 수 있다." | FR-04, FR-06, FR-07, QA-04 (디바이스 plug-in 축은 2단계 — 접속점은 QA-04로 검증) |
...
| R-24 | 과제 발주 (개발 임원·목표 재정의, v1.0) | "과제 범위를 **KV 캐시 최적 운용**으로 특정한다 — ① KV 재사용성을 높여 지연시간을 개선하고 ② LLM 정확도를 유지한 채 KV 압축으로 메모리 병목을 풀어 지연·처리량을 개선하며 ③ KV 캐시 인지형 동적 스케줄링으로 처리량을 높인다. 자사 디바이스 활용(MCR 완성)은 2단계로 한다." | 목표·범위(배경 v5 §3.1), FR-02·03·05 방향, C-03, QA-01 |
