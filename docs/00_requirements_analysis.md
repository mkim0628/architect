...
| **Memory-centric AI System 팀** | 다양한 자사 메모리 제품을 조합한 **시스템을 빌드**하는 부서 — MCR이 올라가는 대상 시스템의 제공자이자 디바이스 조합 효용성 검증의 공동 수요자 | tier 구성(topology) 표현력, tier 조합·P/D 구성 변경 실험의 용이성, 조합별 효용의 정량화 |
| 응용/모델 서비스팀 | 대표 워크로드(long-context RAG · multiturn · agent)의 요구 특성 제공 — 효용성이 입증되면 수혜를 받는 미래 수요처 | 장문 컨텍스트 비용, 세션·장기 기억, 응답 품질 유지 |
| 개발 임원 (랩장) | 과제 승인·자원 배정, 타 과제/사업부 협력 조율 | 연구 가치 — 타겟 메모리의 **효용성을 E2E 정량으로 입증**(baseline 대비), 일정, 지속 유지 가능성 |
| MCR 개발팀 | 런타임 개발·유지보수, 자체 벤치 실측 수행 | 개발 비용, upstream(vLLM) 추종 부담, 코어/모듈 경계 |
...
| 방법 | 대상/출처 | 산출 |
|---|---|---|
| Stakeholder 인터뷰·VOC 접수 | 메모리 사업부 · Memory-centric AI System 팀 · 응용팀 · 임원 · 개발팀 · 고객사(잠재) | 원시 요구사항 R-01~R-22 (부록 A) |
...
- **QA-08 Interoperability** — 독립 QA가 아니라 **DP1(framework 실행
  ...
  2주, 유지 FTE)이 대리 측정.
- **QA-09 Scalability** — 연구 범위가 **고정 N 테스트베드 전제**(§3.1)라
...
| 번호 | 출처 | 내용 | 정제 결과 |
|---|---|---|---|
| R-01 | MCR 개발팀 (자체 실측) | "P/D 분리 벤치에서 decode 대기가 E2E latency의 70–85%다(A) — 토큰 생성이 메모리에 묶여 있다. 타겟 메모리로 이 병목이 풀리는지가 효용성 입증의 핵심이다." | QA-01 |
| R-02 | Memory-centric AI System 팀 | "우리가 빌드하는 이종 메모리 시스템이 HBM 용량 병목(preemption·재계산·스왑)을 실제로 푸는지 — 같은 HBM으로 동시 컨텍스트를 얼마나 더 수용하는지 정량으로 보고 싶다." | QA-02, FR-02, FR-03 |
...
| R-16 | 메모리 사업부 | "기존 KV 계층(LMCache 등)에는 요청별 SLO·품질 예산 기반 정책이 없다(B) — 이 조율 능력이 우리 스택의 차별점이 되어야 한다. 요청마다(interactive vs batch) 배치·압축·재사용 수준을 다르게 가져가라." | FR-06 |
| R-17 | Memory-centric AI System 팀 | "tier 조합·P/D 구성을 바꿔가며 실험한다 — 고정된 노드들 안에서 prefill/decode role 전환·재배분이 자동이어야 구성 변경 실험을 반복할 수 있다." | FR-05, FR-09 |
| R-18 | Memory-centric AI System 팀 | "tier 디바이스 장애로 영속 KV가 유실되면 어떻게 되나 — 세션이 깨지지 않고 복구되어야 한다." | QA-06 |
| R-19 | 고객사 (잠재) | "멀티테넌트 환경에서 사용자 간 KV 재사용이 프롬프트 유출 통로가 되지 않아야 도입할 수 있다 — 격리를 보장하라." | QA-07 |
