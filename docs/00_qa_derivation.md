# 요구사항 → QA 도출 과정 상세 (v0.9)

(v0.9: QA1(TTFT) **이중 기준선** 반영 — 목표 1의 실행 기반을 CacheBlend
채택 후 수정·확장으로 확정(QA 정의 v1.3): 주 기준선 = vLLM+CacheBlend
(수정·확장의 순기여 판정, ★★★ ≥1.3× — 잔여 비용 회수 논증(C)·잠정),
보조 기준선 = 무재사용(효용 확인 ≥2×·문헌 비교·QA2 정합). §2.1 baseline
정의·bin 근거 재작성.)
...
### 2.1 QA1 · QA2. Performance — TTFT(prefill) · throughput(decode) 2 QA

(v0.7: 두 지표를 **독립 QA로 분할** — QA1 TTFT[목표 1] · QA2
throughput[목표 2·3]. 단일 QA 내 AND 판정은 축별 기여·DP 별점 기여를
...
3. **지표 선택 — 왜 TTFT · throughput 2지표인가**:
   ...
   **iso-latency**(TPOT p99 ≤ baseline 운영점, [vLLM SOSP'23](https://arxiv.org/abs/2309.06180)
   ...
   단계 승격 경로로 유지.
4. **baseline 정의와 이유 (v0.9 — QA1은 이중 기준선)**:
   - **QA1 주 기준선 = vLLM + CacheBlend(공개 구현, 동일 HW·동일 구성)**:
     목표 1은 CacheBlend를 **채택해 수정·확장**하는 것이므로, 채택 자체의
     이득(무재사용 대비 2.2–3.3×(B))은 연구 기여가 아니다 — 별점은
     수정·확장의 **순기여**로만 판정한다(C).
   - **보조 기준선(QA1) = QA2 기준선 = 무재사용 GPU HBM 단일 tier 구성**:
     현행 표준 서빙 스택이 달성 가능한 최선 — 효용성 확인(≥2×)·문헌
     비교·ablation 하한으로 병행 보고. P/D 분리는 전제하지 않는다 — 실험
     변수로 두되 전 구성에 동일 적용해 효과를 상쇄시킨다.
5. **bin의 근거 — 두 단계를 각 단계 문헌이 독립 뒷받침**:
   **① TTFT: CacheBlend 대비 ≥ 1.3× AND 무재사용 대비 ≥ 2× (prefill 축 —
   목표 1, v0.9 개정)**:
   무재사용 대비 2×는 [CacheBlend](https://arxiv.org/abs/2405.16444)의
   실측 2.2–3.3×(B)를 채택 기반이 달성하므로 효용 하한 확인 조건(C).
   주 조건 1.3×는 **잔여 비용 회수 논증**(C): CacheBlend 적용 후 잔여
   TTFT의 주성분 = 선택 재계산(HKVD 10–15%(B)) · KV 복원 I/O(로딩-재계산
   중첩 불완전 구간) · 커버리지 밖 요청의 full prefill(multiturn·agent
   세션 KV) — MCR의 수정 지렛대(요청별 재계산율 조정 · 복원 vs 재계산
   판단·tier 배치 · 영속화 커버리지 확대)가 셋에 대응하며, 평균에서
   잔여분 1/4~1/3 회수를 보수 하한으로 두면 ≈1.3×(C). **잠정 — 파일럿
   실측(CacheBlend 재현 + 잔여 TTFT 분해) 후 재캘리브레이션.** 1.1×는
   run-to-run 편차를 넘는 최소 유의 개선 선(C). prefix 재사용 보조 근거:
   [SGLang RadixAttention](https://arxiv.org/abs/2312.07104)(B).
...
### 2.2 QA3. Accuracy — 품질 저하 bound (gate, 우선순위 3)
...
2. **품질 문장 — 왜 '개선 목표'가 아니라 'bound'인가**: 압축(양자화·토큰
   ...
   제약**이며, QA1·QA2·QA4의 수치는 이 bound 안에서만 유효하다. 이 **gate
   성격** 때문에 우선순위도 수단 지표(QA4)보다 앞선다 — 아무리 빨라도
...
4. **왜 throughput(QA2)에 흡수되지 않는가 — 3근거 (v0.8 신설)**:
   "용량↑→batch↑→throughput↑"는 기여 경로 중 하나일 뿐, QA2는 본 QA의
   ...
   ② **재사용 경로** — 영속 KV 보관량 → cache hit rate → TTFT(QA1): 용량은
   QA2만이 아니라 QA1의 상류(C)
...
### 2.5 QA6. Maintainability — 개발·운영 비용 (구 Affordability)
...
## 3. 미선정 5건 — 왜 이 6개"만"인가

선정 논거의 완결성은 탈락 논거에서 나온다(사유 없는 탈락은 stakeholder 배신):

| 후보 | 탈락 논거 | 처리 |
|---|---|---|
| Availability (R-18) | KV는 원본에서 **재계산 가능한 파생 데이터** — 유실은 정확성이 아닌 성능 문제로 환원(C), QA1·QA2 측정에 흡수 | 실패모델은 DP3·DP5 구조 결정으로; 상용화 재평가 |
| Security (R-19) | 실증 단계 재사용 범위를 **사용자/세션 내로 한정**하면 위협 자체가 소거(정책 제약으로 완화) | DP3 커플링; 멀티테넌트 상용화 시 독립 QA로 |
| Interoperability (R-15) | 독립 QA가 아니라 **DP1의 결정 변수** — plugin/독립 선택이 곧 호환성 수준을 결정 | QA5·QA6 bin(upstream+2주, FTE)이 대리 측정 |
...
## 4. 우선순위 산정 — 중요도·난이도 판정 근거 (v0.7 재구성)

규칙: **① 중요도 → ② 동률 시 성능 사슬 내 역할(목표 > 전제 gate > 수단) →
③ 난이도 → ④ 과제 본질 축 우선.** 역할이 난이도에 앞서는 이유: 난이도는
"달성이 어려운가", 우선순위는 "무엇을 먼저 보장해야 하는가" — gate가
무너지면 수단 지표의 수치 자체가 무효가 되므로 검증 의존성 순서가
난이도에 앞선다(C). v0.7 재검수로 H그룹 4건이 모두 H/H 동률 — 순서는
역할 규칙만으로 결정된다.

| QA | 중요도 판정 | 난이도 판정 | 우선순위 |
|---|---|---|---|
| QA1 TTFT | **H** — 목표 1의 최종 판정 지표. re-prefill이 지연의 지배 요인 | **H** — 재사용 이득은 cache-hit·워크로드 의존(B), 비접두 재사용은 품질과 트레이드오프 | 1 (목표 — QA2와 동률, 목표 번호 순) |
| QA2 Throughput | **H** — 목표 2·3의 최종 판정 지표. decode 대기 70–85%(A) 병목의 해소 판정 | **H** — iso-latency 조건 아래 2×, 압축×tier×스케줄링 3중 조율 필요 | 2 (목표) |
| QA3 Accuracy | **H** — QA1·QA2·QA4의 전제(gate): bound 위반 시 성능·용량 수치 무효. 목표 2 문장("정확도 유지")의 일부 | **H** (v0.7 M→H) — 차등 압축·비접두 재사용·축출의 3중 품질 노출을 요청별 bound로 통합 추적 + training-free(C-03)로 회복 수단 배제 | 3 (gate — 목표 다음, 수단 앞) |
| QA4 Resource Efficiency | **H** — HBM당 컨텍스트가 비용 구조 결정, 이종 tier의 존재 이유 | **H** — 압축 단독(1.5×, 문헌 입증)을 넘어 품질 bound 안에서 tier 결합으로 3× | 4 (수단) |
| QA5 Modifiability | **M** — 초기 가치 실증엔 비직결, 중장기 생존성 좌우. KV 구조 변화는 과제 본질에 대한 변화 | **H** — 코어/모듈 경계는 되돌리기 어려운 초기 결정; MLA·linear attention 수용은 어려움 | 5 |
| QA6 Maintainability | **M** — 지속 가능성 좌우하나 성패 지표 아님 | **M** — plugin 경계 유지 시 관리 가능(02 실측) | 6 |

## 5. PPT 반영 가이드 (P7 슬라이드)

- 헤드라인: "품질 발화 10건 → 시나리오화 → Utility Tree(후보 11) → **QA
  6건 선정**" (§0 5단계를 소형 흐름도로. 선정 6 = 실질 관심축 5 —
  Performance 2분할).
...
- 미선정 5건은 슬라이드 하단 1줄씩(§3 표의 탈락 논거 열).
