...
- 예: R-03 "chunk KV를 재사용하고 싶다" → 재사용이라는 **기능**(FR-04).
  ...
  하는가의 상한 → **품질**(→QA3). R-10 "디바이스 HW 스펙은 못 바꾼다" →
...
### 2.3 QA3. Accuracy — 품질 저하 bound (gate, 우선순위 2)
...
2. **품질 문장 — 왜 '개선 목표'가 아니라 'bound'인가**: 압축(양자화·토큰
   ...
   제약**이며, QA1·QA2의 수치는 이 bound 안에서만 유효하다. 이 **gate 성격**
   때문에 우선순위도 수단 지표(QA2)보다 앞선다 — 아무리 빨라도 품질이
