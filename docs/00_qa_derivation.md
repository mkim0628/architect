...
## 4. 우선순위 산정 — 중요도·난이도 판정 근거 (v0.9 rubric 재구성)
...
우선순위 규칙: **① 중요도 → ② 동률 시 성능 사슬 내 역할(목표 > 전제
gate > 수단) → ③ 난이도.** 역할이 난이도에 앞서는 이유: 난이도는 "달성이
어려운가", 우선순위는 "무엇을 먼저 보장해야 하는가" — gate가 무너지면
수단 지표의 수치 자체가 무효가 되므로 검증 의존성 순서가 난이도에
앞선다(C). **목표 간(QA1↔QA2)에는 난이도 서열을 적용하지 않는다** — 종합
판정이 AND 조건이라 목표 간 서열은 무결정력, 순번은 목표 번호 순(C).

| QA | 중요도 판정 (I1·I2·I3) | 난이도 판정 (D1·D2·D3) | 우선순위 |
|---|---|---|---|
| QA1 TTFT | **H** (5 = 2·1·2) — 목표 1 단독 판정·전용 VOC(R-03·04·05) | **M 상위** (3 = 1·1·1, v0.9 H→M) — 경로 2/3가 문헌 재현(CacheBlend·SGLang), 신규는 agent memory 축(복원 vs 재계산)에 국한 | 1 (목표 — 순번은 목표 번호 순) |
| QA2 Throughput | **H 최고** (6 = 2·2·2) — 목표 2·3 공동 판정(미달 시 복수 목표 실패)·decode 병목 70–85%(A) 해소 판정 | **H** (5 = 2·2·1) — KV 공간 확보 선택 정책은 문헌 공백 + iso-latency×3메커니즘×ablation 동시 충족 | 2 (목표) |
| QA3 Accuracy | **H** (5 = 1·2·2) — gate: 전 QA 수치 무효화(파급 최대)·R-06 전용 | **H 최고** (6 = 2·2·2) — 요청별 bound 집행·3중 추적은 문헌 공백 + training-free로 회복 수단 부재 | 3 (gate — 목표 다음, 수단 앞) |
| QA4 Resource Efficiency | **H** (5 = 2·1·2) — **목표 2 공동 판정**: 기전(병목 해소) 미입증 시 처리량이 올라도 인과 불성립 = 목표 2 실패 + R-02 대체 불가 | **M** (3 = 1·2·0, v0.9 H→M) — pruning 확정으로 도달선 문헌화(H2O 20% = 5×(B)), 잔여는 3중 제약 하 운용 튜닝(반복 조정 가능) | 4 (수단) |
...
**변별력 메모**: H/H는 **QA2·QA3 2건뿐** — QA2는 중요도 최고점(6),
QA3는 난이도 최고점(6)으로 축이 다르다. QA1·QA4는 중요도 H·난이도 M으로
"중요하지만 도달 경로가 문헌에 있는" 축, QA5는 그 역(M/H — 덜 급하지만
되돌리기 어려운 축)이다.
