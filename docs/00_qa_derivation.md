...
2. **품질 문장 — 왜 '개선 목표'가 아니라 'bound'인가**: 압축(양자화·토큰
   ...
   **압축** + **비접두 재사용**(선택 재계산율 트레이드오프, CacheBlend
   0.01–0.03(B)) + 스케줄링의 **축출/강등 선택**의 3중이 되었고, 요청별
   bound 집행은 이를 통합 추적하는 구조를 요구하며, C-03(training-free)으로
   재학습 기반 회복 수단이 배제된다(C) — 문헌 재현(M)을 넘는 설계 부담.
