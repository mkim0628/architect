...
| **Accuracy** (QA3. 응답 품질) | 압축·재사용 품질 저하 상한(bound) — 성능·용량 수치의 유효 전제(gate) | 압축(양자화·토큰 eviction)·재사용을 실서빙 설정으로 활성화하고 long-context 벤치마크(LongBench 등)에서 baseline(비압축 FP16 KV·비재사용 — 품질의 이론적 상한) 대비 F1-score 차이(ΔF1)를 측정하고, bound 집행 단위(요청별 vs 전역)를 판정한다. 보조: ΔPPL(Wikitext-2). [측정: ΔF1 ≤ 1%p · 요청별 bound 집행 → ★★★] | H | M | 2 |
| **Resource Efficiency** (QA2. 메모리 효율) | 유효 KV 용량 (원본 환산 동시 수용량) | QA3 품질 bound를 지키는 조건에서 Σ_tier(용량 × 평균 압축률 × KV 가용 비율)로 원본 환산 유효 KV 용량을 산출하고 물리 HBM 용량 대비 배율을 구한다. baseline = HBM 단일 tier·비압축(정의상 1.0× — HBM당 수용 컨텍스트가 비용 결정). [측정: 유효 KV 용량 ≥ 3× → ★★★] | H | H | 3 |
| **Modifiability** (QA4. 확장성·진화성) | 신규 메모리 디바이스·KV 구조 변화 수용 용이성 | (a) 신규 tier(HBM4/CMM-DC/HBF) 추가 시 신규/변경 모듈 수, 코어 변경 LOC 비율(%), 공개 인터페이스 시그니처 변경 건수를 세고 (b) KV 구조 영향 모델 변화(GQA/MQA·MLA·sliding-window·SSM)의 수용 리드타임을 upstream 공개 시점 기준으로 잰다. baseline = 현행 코드베이스. [측정: 신규 어댑터 모듈 ≤ 1 · 코어 변경 LOC 0 · 시그니처 변경 0건 · 수용 ≤ upstream + 2주 → ★★★] | M | H | 4 |
...
- **우선순위 산정**: ① **중요도** 우선 ② 동률이면 성능 사슬 내 **역할** —
  ...
  QA3(품질 bound — bound 위반 시 성능·용량 수치 무효인 gate) > QA2(유효 KV
  용량, 수단) ③ 그 외 동률은 **난이도** (QA4·QA6 M/H > QA5 M/M) ④ 남는
  동률은 과제 본질 축 우선 — QA4(타겟 메모리·모델 수용)가 QA6(framework
  교체 헤지)에 우선(C).
- **QA 번호는 도입 순 식별자로 고정**한다 (v0.4까지 부여된 번호 유지 + 신설은
  끝 번호). 우선순위와 번호를 분리하는 이유: 우선순위는 평가에 따라 변하는데
  번호를 따라 바꾸면 타 문서(DP 평가표 등)의 QA 번호 참조가 모두 무효화되기
  때문. 우선순위는 본 표의 우선순위 열과 각 QA 절의 우선순위 필드가 나타낸다.

**개정 이력**
- v0.5: 요구사항 분석 v0.4 리뷰 반영 — ① QA1 baseline에서 P/D 분리 전제
  제거(동일 실행 구성의 GPU HBM 단일 tier로 재정의, 선정 이유 명시)
  ② QA3 주지표를 accuracy(%p)에서 **F1-score(ΔF1)** 로 전환(LongBench QA
...
- **우선순위: 3** — 중요도 H 그룹 내 **수단 지표** — 목표(QA1)와 그 유효
  전제(QA3 gate) 다음 (v0.5에서 QA3과 순위 교환).
...
- **우선순위: 2** — 중요도 H 그룹 내 **전제(gate)** — 성능·용량 수치의
  유효 조건이므로 수단 지표(QA2)에 앞선다 (v0.5에서 상향).
...
- **정의**: 압축·재사용으로 인한 품질 저하의 상한 보장 — QA1·QA2 수치의
  유효 전제(gate).
...
- **ΔF1 ≤ 2%p (중간 bin)**: KIVI 2-bit(최대 압축단)의 LongBench 실측 저하
  ...
  문헌이 보인 저하 한계 이내여야 함.
- **ΔF1 ≤ 1%p (상위 bin)**: 02 문서의 기존 사내 기준 1%p(A)를 accuracy에서
  F1로 이관 + KV 압축 문헌의 near-lossless 관례 허용선(B). 극한 압축단
  실측 상한(2%p)의 절반으로 잡아 압축단 선택의 여지를 남긴 보수선(C).
...
- **평가 시나리오**: (a) 신규 메모리 tier(HBM4/CMM-DC/HBF) 1종 추가 실험에서
  **신규/변경 모듈 수 · 코어 변경 LOC 비율(%) · 공개 인터페이스 시그니처
  ...
  시점 기준으로 측정한다. baseline = 현행 코드베이스.
