...
|---|---|---|---|---|---|
| **Performance** (QA1. 추론 성능) | 지연 가드 내 최대 처리율 (baseline 대비 throughput 배율) | 대표 워크로드(long-context RAG · multiturn · agent memory)를 동일 HW·동일 실행 구성에서 서빙하며, **retrieval(유사도 검색 — SSD-PIM 근접 가속, ADR-001)을 포함한 E2E 경로**의 처리율(throughput)을 잰다. 지연 가드(TPOT p99 ≤ baseline의 1.5×) 안에서 측정하고 throughput–latency 곡선을 병행 보고한다. baseline = GPU HBM 단일 tier 구성(타겟 메모리 없이 달성 가능한 최선 — 순증분 분리 측정). [측정: baseline 대비 throughput ≥ 2× → ★★★] | H | H | 1 |
...
- **우선순위 산정**: ① **중요도** 우선 ② 동률이면 성능 사슬 내 **역할** —
  최종 목표 > 그 유효성의 전제(gate) > 수단. QA1(처리율, 목표) >
...
| KV 재사용 처리율 (RAG) | RAG KV 재사용 + 선택 재계산(HKVD 10–15%): **TTFT 2.2–3.3× 단축 · 처리율 2.8–5×**, 품질 저하 F1/Rouge-L 0.01–0.03 | [CacheBlend (EuroSys'25 Best Paper)](https://arxiv.org/abs/2405.16444) |
| KV 재사용 처리율 (prefix 공유) | RadixAttention cross-request 재사용 — multiturn·RAG·few-shot에서 **최대 6.4×** 처리율 | [SGLang (NeurIPS'24)](https://arxiv.org/abs/2312.07104) |
| near-storage 검색 가속 | SSD 기반 ANN에서 I/O가 실행 시간 ~67% → host CPU + SSD 협력 인덱싱으로 **QPS 최대 10.7×** | [SmartANNS (ATC'24)](https://www.usenix.org/system/files/atc24-tian.pdf) · [ADR-001](adr/ADR-001-ssd-pim-rag-retrieval.md) |
...
**MCR 표준 지연 가드**(별도 명시 없으면 이 조건으로 측정): **TPOT p99 ≤
baseline의 1.5×** 이내에서 처리율을 측정하고, throughput–latency 곡선을 병행
보고한다(v0.8 — 절대 SLO에서 상대 가드로 전환). 대표 워크로드 = long-context
RAG · multiturn · agent memory (과제 범위 3.2-3). 절대 SLO(MLPerf TTFT
450 ms 등)는 QA1의 측정 조건이 아니라 **참고 앵커**다 — 상용화 단계의
goodput@SLO 승격(아래 QA1 주) 및 DP7의 TTFT 예산 논의에서 참조한다.

---

## QA1. 추론 성능 (baseline 대비 throughput — 지연 가드 내)

- **중요도: H** — MCR의 존재 목적이 "지연을 무너뜨리지 않으면서 처리율을
  올리는 것"이며, 과제의 최종 판정 지표. 이 QA 미달이면 나머지 QA가 모두
  우수해도 무의미(C).
- **난이도: H** — 이종 tier 오케스트레이션·압축·재사용·근접연산 검색이 모두
  맞물려야 2×에 도달하며, decode wait 70–85%(A) 병목을 실제로 해소해야 함.
  재사용 이득은 cache-hit·워크로드 의존(B)이 커서 대표 워크로드 전반에서
  bin을 지키기 어려움.
...
- **평가 시나리오**: 동일 HW·동일 실행 구성에서 GPU HBM 단일 tier
  ...
  측정한다 — SSD-PIM 근접 검색 가속(ADR-001)의 기여가 지표에 포함되도록.
  부하(동시 요청 수)를 스윕해 throughput–latency 곡선을 얻고, **지연
  가드(TPOT p99 ≤ baseline의 1.5×) 안에서의 최대 처리율**을 baseline과
  비교한다. [측정: baseline 대비 throughput ≥ 2× → ★★★]

- **정의**: 지연 가드 안에서 달성하는 최대 처리율(throughput)의 baseline
  대비 배율.
- **측정**: throughput(tokens/s 또는 req/s)의 baseline 대비 배율 — **지연
  가드: TPOT p99 ≤ baseline의 1.5×** 이내의 운영점에서 측정하고,
  **throughput–latency 곡선을 병행 보고**한다(배율이 특정 운영점의
  체리피킹이 아님을 곡선으로 입증). 판정은 가드 내 배율, 보고는 곡선 —
  둘을 병행한다.
...
> **주 — 왜 goodput@SLO가 아닌가 (v0.8)**: 절대 SLO(MLPerf TTFT 450 ms ·
> TPOT 200 ms)는 벤치마크 시나리오에 고정된 제약이라, GPU 종류·규모가
> 비확정인 연구 테스트베드에서는 구성이 약하면 어떤 처리율에서도 미달
> (goodput = 0)로 지표가 퇴화한다. 실서빙 SLA가 없는 연구 과제 성격상
> 배율 지표가 문헌(vLLM·KIVI·FlexGen·CacheBlend)과의 비교성도 높다. 단
> 지연 제약을 전부 없애면 FlexGen형(수십 초 지연의 offline 처리량 100×)
> 퇴화 구성이 이기는 지표가 되므로 **상대적 지연 가드**만 남긴다.
> 상용화 단계(실서빙 SLO 계약 확정 시)에는 goodput@SLO(DistServe (B))로
> 승격한다.

| 별점 | 기준 (지연 가드 TPOT p99 ≤ baseline 1.5× 내, baseline 대비 throughput) |
|---|---|
| ★★★ | **≥ 2×** — 결합(압축×재사용×tiering×근접연산)의 부가가치 입증선 |
| ★★☆ | 1.5× – 2× — 단독 기법(압축만 등)으로도 도달 가능한 수준, 결합 순증분 미입증 |
| ★☆☆ | < 1.5×, 또는 지연 가드 위반 구성으로만 달성 |

**bin 근거 (2×)**: MCR이 결합하는 **각 축의 단독 실측이 모두 2×를 넘는다** —
① 압축: KIVI 2-bit, 처리율 **2.35–3.47×**(B) ② KV 재사용: CacheBlend(RAG
KV 재사용+선택 재계산), 처리율 **2.8–5×**(B, EuroSys'25 Best Paper) ·
SGLang RadixAttention(prefix 공유), **최대 6.4×**(B) ③ 메모리 관리 단독:
vLLM PagedAttention, 동일 GPU 처리량 **2–4×**(B) ④ retrieval 구간:
SmartANNS(near-storage 협력 인덱싱), QPS **최대 10.7×**(B) — RAG E2E에서
검색 지연 단축이 처리율에 기여(ADR-001). 서로 다른 병목에 작용하는 축들을
결합하는 시스템에 **단독 축 실측 범위의 하단(2×)** 을 요구하는 것은
보수적(C). 반면 1.5×는 압축 단독의 보수 하한이므로 결합의 부가가치
입증선이 못 된다 — ★★☆로 강등(C). 각 축의 이득은 cache-hit·워크로드
의존(B)이므로 대표 워크로드 3종 전반에서 bin 충족을 요구한다. **지연 가드
1.5×**: 처리량–지연 트레이드에서 offline-throughput형 퇴화 구성(FlexGen
100×는 지연 수십 초(B))을 배제하는 완만한 상한(C) — 컷라인의 임의성은
곡선 병행 보고로 보완한다. *주의: DistServe의 2–7.4×는 colocated → P/D
분리 전환의 효과로, 실행 구성을 양쪽에 동일하게 두는 본 QA의 bin 근거가
아니다.*
...
- **중요도: H** — HBM이 희소 자원이며 "HBM 한 장당 서빙 가능한 컨텍스트"가
  비용 구조를 결정. QA1(처리율) 달성의 직접 수단이자 이종 tier 도입의
...
- [CacheBlend: Fast Large Language Model Serving for RAG with Cached Knowledge Fusion (EuroSys'25 Best Paper)](https://arxiv.org/abs/2405.16444) (RAG KV 재사용 — TTFT 2.2–3.3× 단축, 처리율 2.8–5×, 품질 저하 0.01–0.03 — QA1 2× bin 근거)
- [SmartANNS: Efficient Billion-Scale ANN Search with SmartSSDs (ATC'24)](https://www.usenix.org/system/files/atc24-tian.pdf) (SSD ANN I/O 병목 ~67%, 협력 인덱싱 QPS 최대 10.7× — QA1 retrieval 축·ADR-001 근거)
