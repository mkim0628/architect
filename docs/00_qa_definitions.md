...
| **Resource Efficiency** (QA4. 메모리 효율) | 유효 KV 용량 (원본 환산 동시 수용량) | QA2 품질 bound를 지키는 조건에서 Σ_tier(용량 × 평균 압축률 × KV 가용 비율)로 원본 환산 유효 KV 용량을 산출하고 물리 HBM 용량 대비 배율을 구한다. baseline = HBM 단일 tier·비압축(정의상 1.0× — HBM당 수용 컨텍스트가 비용 결정). [측정: 유효 KV 용량 ≥ 3× → ★★★] | H (용량 시나리오 공동 판정 — 기전 입증) | M (v1.3 H→M — rubric 3점) | 4 |
...
**세부 판정 기준 — 등급 변별 rubric (v1.3 신설)**: "전 QA가 H/H"는 판정의
변별력 부재로 공격받는다는 검수 반영 — 각 축을 3개 세부 기준 × 0/1/2점으로
채점하고 **H ≥ 5 · M 2–4 · L ≤ 1**로 등급화한다.

- **중요도**: **I1 핵심 시나리오 직결성** (v1.6 — 구 "목표 직결성" 재정의:
  채점 입력은 요구사항 분석 §4.1의 대표 워크로드 시나리오와 VOC로 한정,
  과제 목표 문장은 참조하지 않는다. 2 = 핵심 시나리오를 단독/관통 판정 ·
  1 = 시나리오의 조건·기전을 정량 · 0 = 판정 무관) / **I2 파급 범위** (2 = 타 QA
  수치 무효화[gate] 또는 복수 목표 실패 · 1 = 단일 목표 실패 또는 산출물
  수명·2단계 연결 훼손 · 0 = 없음) / **I3 VOC 대체 불가성** (2 = 전용
  VOC 존재·타 지표로 대체 불가 · 1 = 부분 대체 가능 · 0 = 간접)
- **난이도**: **D1 신규 설계 필요성** (2 = ★★★ 핵심 경로가 문헌 공백 ·
  1 = 일부 축만 신규 · 0 = 문헌 재현+조합) / **D2 동시 충족 폭** (2 = 3중
  이상 결합 · 1 = 2중 · 0 = 단일) / **D3 회복·가역성** (2 = 불가역 초기
  결정·회복 수단 부재 · 1 = 조정 가능하나 고비용 · 0 = 반복 조정 가능)

| QA | I1·I2·I3 → **중요도** | D1·D2·D3 → **난이도** |
|---|---|---|
| QA1 Throughput | 2(용량·decode·스케줄링 시나리오 관통 판정)·2(복수 시나리오 요구 실패)·2(R-01·11·16) = **6 → H (최고)** | 2(KV 공간 확보 선택 정책 — 문헌 공백)·2(iso-latency×3메커니즘×ablation)·1(중앙/자율 구조 결정 부담) = **5 → H** |
| QA2 Accuracy | 1(시나리오의 품질 조건절)·2(전 QA 수치 무효 — gate)·2(R-06) = **5 → H** | 2(요청별 bound 집행·3중 추적 — 문헌 공백)·2(3영향원×요청별×전 워크로드)·2(training-free — 회복 수단 부재) = **6 → H (최고)** |
| QA3 TTFT | 2(RAG·multiturn·agent 시나리오 판정)·1(해당 시나리오군 실패)·2(R-03·04·05) = **5 → H** | 1(agent memory 축만 신규)·1(워크로드 3종 전반)·1(KV 인덱스·저장 구조 변경 부담) = **3 → M** |
| QA4 Resource Eff. | 2(용량 시나리오 **공동 판정** — 기전 입증)·1(단일 시나리오군 실패)·2(R-02) = **5 → H** | 1(재사용 KV 보수 pruning 제약 하 압축률 정책만 신규)·2(bound×충돌×가용률 3중 제약)·0(운용 튜닝 — 반복 조정 가능) = **3 → M** |
...
- **우선순위 산정 (v1.4 개정)**: ① **중요도**(rubric 점수) → ② **난이도**
  (rubric 점수) → ③ 잔여 동률은 성능 사슬 내 **역할**(시나리오 최종 판정 지표 > gate > 수단·기전). 채점 입력은 요구사항(§4.1 시나리오·VOC)으로 한정한다(v1.6).
  적용: QA1 throughput(중요도 6 — 단독 최고) → 중요도 5점 동률군은
  난이도로 — QA2 Accuracy(난이도 6) > {QA3 TTFT, QA4}(각 난이도 3) →
  잔여 동률(5·3)은 역할로 QA3(시나리오 최종 판정) > QA4(기전) → M그룹은
  QA5(M/H)만 잔존(Maintainability는 v1.5 미선정 전환). gate가 2위인 것은 강등이 아니다 — gate의 파급(전
...
**개정 이력**
- v1.6: **rubric 채점 기준을 요구사항 기점으로 재정렬 (주객전도 교정
  검수 반영, 요구사항 분석 v1.6과 정합).** ① 중요도 **I1 "목표 직결성" →
  "핵심 시나리오 직결성"** 재정의 — 채점 입력을 요구사항 분석 §4.1의
  대표 워크로드 시나리오·VOC로 한정하고 과제 목표 문장의 참조를 금지
  ② 채점표·각 QA 중요도 서술의 "목표 n" 앵커를 시나리오·VOC 앵커로 교체
  ③ 방향 명시: 과제 목표의 정량 목표(Exit Criteria)는 선정 QA의 ★★★
  bin에서 역으로 정의된다(요구사항 분석 §4.5). **전 QA 재채점 결과
  점수·등급·우선순위·bin 불변** — 근거의 출처만 교정
- v1.5: **QA6(Maintainability) 미선정 전환** — 초기 구축 인월·유지 FTE는
  연구 과제 성패의 critical 요구사항이 아니라는 검수 반영(rubric에서도
  중요도·난이도 모두 최하 2점). 선정 QA는 **5건(QA1–QA5)으로 과제
  표준(상위 5개 선정) 복귀**. 구 bin(초기 ≤6인월·유지 ≤0.5 FTE)은
  삭제가 아니라 **DP1(실행 스택/플랫폼 결정)의 비용 모델 판단 기준으로
  강등·보존** — R-08(레퍼런스 스택)·R-12(소수 인력 유지)의 착지점은 그
  판단 기준과 상용화 단계 재평가로 유지
- v1.4: **우선순위 규칙 개정(중요도 → 난이도) + 재번호.** 역할 tie-break를
  ...
  **TTFT QA1→QA3**(동률군 난이도 3 — QA4와 완전 동률, 잔여 동률만 역할
  [목표>수단]로 판정) / QA4·QA5 불변. gate(Accuracy)가 목표
...
- v1.3: **등급 변별 rubric 도입 + 중요도·난이도 재판정** ("전 QA H/H"는
  ...
  충족 폭·D3 회복·가역성) × 0/1/2점으로 채점하는 rubric 신설 — 등급마다
  점수 근거 명시 ② **QA1 난이도 H→M(상위)**: ★★★ 경로의 2/3가 문헌
  재현(RAG=CacheBlend 2.2–3.3×(B)·multiturn=prefix 재사용(B)), 신규 설계는
  agent memory 축(복원 vs 재계산 판단)에 국한 ③ **QA4 난이도 H→M**:
  pruning 확정(v1.2)으로 도달선이 문헌화(H2O 예산 20% = 산술 5×(B)) —
  잔여는 3중 제약 하의 운용 튜닝(반복 조정 가능) ④ **QA4 중요도 H 유지 —
  근거 재구성**: 단순 "수단"이 아니라 **목표 2의 공동 판정 지표** — 목표
  2 문장이 수단(압축)–기전(병목 해소)–결과(지연·처리량)를 모두 명시하므로
  결과(QA2)만으로 목표 2를 판정할 수 없고 기전(유효 KV 용량)이 함께
  입증되어야 함(미달 시 인과 불성립 = 목표 2 실패) ⑤ 우선순위 규칙 보강:
  **목표 지표 간 난이도 서열 비적용**(AND 조건 — 순번은 목표 번호 순).
  결과: 등급 분포 H/H 4건 → **2건(QA2·QA3)**, 우선순위 1–6 불변, bin 불변
...
(v1.1 — 구 QA1 "추론 성능"의 throughput 축을 독립 QA로 분리. **용량·decode·
스케줄링 시나리오(요구사항 §4.1)의 관통 판정 지표 — 목표 2·3의 Exit
Criteria가 본 bin에서 정의된다(§4.5).** QA3(TTFT)과 쌍 — 종합 판정에서 두 QA 모두 ★★★이어야 구 통합 Performance ★★★와 동치. v1.4 — 우선순위 규칙 개정[중요도→난이도]으로 QA2→QA1 재번호: 중요도 rubric 최고점.)

- **중요도: H** — 용량·decode·스케줄링 시나리오 3건(§4.1, R-01·11·16)이
  수렴하는 관통 판정 지표. decode는
...
- **우선순위: 1** — 중요도 rubric **최고점(6)** — 미달 시 복수 시나리오 요구(R-01·11·16)가 동시에 실패하는 유일한 QA. 규칙 ①(중요도)만으로 단독 1위.
...
- **중요도: H** — QA1(throughput)·QA3(TTFT)·QA4의 전제 조건: 품질 bound를
  ...
  bound를 지키는 조건에서만 인정"). R-06이 품질 저하의 상한 보장을 명시적으로
  요구하므로 gate가 곧 요구사항이다(목표 2의 "정확도 유지" 문구는 이
  요구의 수용). 서비스 품질 저하는 사용자에게
  직접 노출되는 리스크(C).
...
(v1.1 — 구 QA1 "추론 성능"의 TTFT 축을 독립 QA로 분리. **KV 재사용
시나리오 축의 판정 지표 — 목표 1의 Exit Criteria가 본 bin에서 정의된다.** v1.4 — 우선순위 규칙
개정으로 QA1→QA3 재번호.)

- **중요도: H** — RAG·multiturn·agent 시나리오(R-03·04·05)의 사용자 체감
  판정 지표.
  장문 컨텍스트의 매 요청 전체 re-prefill(R-03·04)이 지연의 지배 요인이며,
  TTFT는 사용자가 체감하는 첫 지표다. 미달이면 재사용 시나리오군의 요구가 실패한다(C).
...
- **중요도: H** (v1.6 — 요구사항 앵커: **용량 병목 시나리오의 공동 판정
  지표**) — 용량 시나리오(§4.1)의 요구는 결과(처리량↑)만이 아니라
  **기전의 재현 검증**이다: R-02(MCAS)가 "같은 HBM으로 동시 컨텍스트를
  얼마나 더 수용하는지 정량으로"를 명시적으로 요구한다. 따라서 결과
  지표(QA1 throughput)만으로는 이 시나리오를 판정할 수 없고, **기전(병목
  해소 = 유효 KV 용량)이 함께 입증되어야 한다** — 본 QA 미달 시 처리량이
  올라도 "병목을 풀어서 올랐다"는 인과가 성립하지 않아 해당 시나리오
  요구는 실패로 판정된다(I1=2, rubric 5점). 여기에 throughput으로 환원되지 않는 독립 가치가 셋
  있다(v1.2):
...
- **난이도: M** (v1.3 H→M 재판정 — pruning 확정의 후속 효과) — pruning
  방향 확정(v1.2)으로 **도달선이 문헌에 생겼다**: KV 예산 20%(H2O(B))는
  산술 5×이고 tier 확장이 이를 더 키운다. 신규 설계는 재사용 KV 보수
...
> **각주 — 유지보수성(구 QA6 Maintainability)의 처리 (v1.5 미선정
> 전환)**: 초기 구축 인월·연간 유지 FTE는 v1.5 검수에서 **critical
> 요구사항이 아님**으로 판정 — rubric 중요도 2점(성패 지표 아님)·난이도
> 2점(표준 산정 방법)으로 선정 QA 중 유일하게 양축 최하였다. 독립 QA에서
> 제외하되 구 bin(★★★ 초기 ≤ 6인월 · 유지 ≤ 0.5 FTE / ★★☆ ≤ 24인월 ·
> ≤ 2 FTE — 02 문서 실측 표현(A) 기반)은 **DP1(실행 스택/플랫폼 결정)의
> 비용 모델 판단 기준으로 보존**한다 — 구조 선택이 비용을 한 자릿수 이상
> 가르는 사실 자체는 유효하기 때문. R-08(레퍼런스 스택 제공)·R-12(소수
> 인력 유지)는 이 판단 기준과 상용화 단계 재평가로 처리(요구사항 분석
> §4.2 미선정 사유 참조).
