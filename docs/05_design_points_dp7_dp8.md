...
> throughput(decode 축) 각 ≥ 2×**로 개정됨에 따라 QA1 행을 재채점. DP7(검색
> 실행)·DP8(검색 계약)은 모두 **TTFT 축**(retrieval 가속 · prefill 재계산
> 회수)에 작용하므로 그 축의 배율을 신 bin으로 판정하고 throughput 축은
> tier·압축 DP 책임임을 병기한다 (상세 규칙: 02 문서 §0 주석).
...
2. **recall의 품질 환산**: ANN recall@k 손실(놓친 chunk)이 응답 품질(QA2)에
...
|----|------|-----------|
| QA1 | ★★☆ (F) | **TTFT 축(retrieval 구성분)**: 소·중규모 코퍼스에선 내부 대역폭 스캔으로 검색 지연 단축 — TTFT 기여 1.5–2×(C). 코퍼스가 임계(≈내부 유효 BW × 예산) 초과 시 지연 선형 증가로 TTFT 2× 미달·규모 조건부(C). throughput과 직교. 종합 ★★☆ |
| QA2 | ★★★ (F) | recall 100% — 검색발 응답 품질 저하 0%p, bound 문제 자체가 없음(C) |
| QA3 | ★★★ (F) | 벡터는 SSD 상주 — HBM 점유 0. *본 QA는 두 후보를 가르지 않음* (KV 용량과 직교)(C) |
...
| QA | 평점 | 정량 근거 (00 v0.3 bin 판정) |
|----|------|-----------|
| QA1 | ★★★ (상한) / ★★☆ (도달 리스크) (F) | **TTFT 축(retrieval 구성분)**: sublinear 스캔으로 billion-scale에서도 검색 TTFT 예산 내 — QPS 최대 10.7×(B: SmartANNS), retrieval 구성분 **2× 초과 경로**. 도달 리스크: recall-지연 트레이드 튜닝과 host 왕복 오버헤드에 종속(C). throughput과 직교 |
| QA2 | ★★☆ (F) | recall@k < 1.0의 chunk 누락이 응답 품질 저하로 전이 — ANN 관례 recall 0.9~0.99(B)에서 전역 bound 운용은 가능하나 요청별 보장 불가(C) |
| QA3 | ★★★ (F) | *비차별* — 동일(C). (host 상주 인덱스의 DRAM 점유는 실재하나 QA3 정의(HBM 분모) 밖 — 검토 노트 참조) |
...
| QA | 평점 | 정량 근거 (00 v0.3 bin 판정) |
|----|------|-----------|
| QA1 | ★★☆ (F) | **TTFT 축(prefill)**: 검색 지연 단축분만 회수 — retrieval hit 후 prefill 재계산 경로 잔존으로 TTFT의 지배 몫(재계산) 미회수, TTFT 1.5–2×(C). throughput과 직교. 종합 ★★☆ |
| QA2 | ★★★ (F) | 재사용 경로에 새 근사 없음 — 검색발 품질 영향 0 (DP7 채택안의 recall 특성만 상속)(C) |
| QA3 | ★★☆ (F) | 1.5–3× bin: 벡터·텍스트·KV 3중 저장 중복 잔존 — co-location dedup 없이 압축·tier 이득만(C) |
...
| QA | 평점 | 정량 근거 (00 v0.3 bin 판정) |
|----|------|-----------|
| QA1 | ★★★ (상한) / ★★☆ (도달 리스크) (F) | **TTFT 축(prefill)**: hit 시 prefill 재계산 소멸 — 비연속 재사용 TTFT 2.2–3.3×(B: CacheBlend)와 융합, **2× 초과 경로**. 도달 리스크: SSD→GPU KV 승격 대역폭과 DP3 후보2 채택에 종속(C). throughput과 직교 |
| QA2 | ★★☆ (F) | KV 재사용 경로 확대 = 위치 근사·selective recompute 개입(DP3 종속) — 전역 bound 운용(C), 요청별 집행은 DP2 채택안에 종속 |
| QA3 | ★★★ (F) | ≥3× bin 경로: 텍스트/벡터/KV co-location dedup + chunk 전역 재사용 — 원본 환산 수용량 극대(C) |
