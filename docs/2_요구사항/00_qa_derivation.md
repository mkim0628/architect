# 요구사항 → QA 도출 과정 상세 (v0.12)

(v0.12: **우선순위 근거를 요구사항 기점으로 재정렬** — 요구사항 분석
v1.6·QA 정의 v1.6과 정합. 본 문서의 도출 체인에서 "목표 n"은 채점
근거가 아니라 **결과 참조**로만 읽는다: 채점(중요도 I1 = 핵심 시나리오
직결성)의 입력은 대표 워크로드 시나리오(요구사항 분석 §4.1)와 VOC이며,
목표의 정량 목표(Exit Criteria)는 선정 QA bin에서 역정의된다(§4.5).
체인·점수·우선순위 불변.)

(v0.11: **QA6 Maintainability 미선정 전환** — 초기 구축 인월은 critical
요구가 아님(rubric 양축 최하 2점). 선정 5건(QA1–QA5)으로 과제 표준 복귀.
§2.5 체인은 §3 미선정 표로 이동, bin은 DP1 비용 모델 판단 기준으로 보존.)
(v0.10: **우선순위 규칙 개정(① 중요도 → ② 난이도 → ③ 잔여 동률은
역할)** + 재번호(QA 정의 v1.4) — throughput QA2→QA1(중요도 6 최고),
Accuracy QA3→QA2(동률군 난이도 6 최고), TTFT QA1→QA3(QA4와 완전 동률 —
잔여 동률만 역할[목표>수단]). §2 절 번호·§4 표 재구성. 정의·bin 불변.)
(v0.9: **등급 변별 rubric**(QA 정의 v1.3) 반영 — 중요도 I1·I2·I3 /
난이도 D1·D2·D3 × 0/1/2점 채점으로 §4 재구성. QA1 난이도 H→M(경로 2/3
문헌 재현, agent memory 축만 신규), QA4 난이도 H→M(pruning 확정으로
도달선 문헌화), QA4 중요도 H 유지 — 근거를 "목표 2 공동 판정(기전
입증)"으로 재구성. 목표 간 난이도 서열 비적용 규칙 신설. 우선순위
1–6·bin 불변 — H/H는 QA2·QA3 2건으로 정돈.)
(v0.8: 압축 기법 방향 확정(중요도 기반 토큰 pruning 중심) 반영 — §2.1
QA2 bin 근거 1차 앵커를 H2O·SnapKV로 교체(bin 수치 불변), §2.2 압축
표기 재정렬 + pruning×재사용 충돌을 요청별 집행 조건 근거에 추가, §2.3
QA4 체인에 "왜 QA2에 흡수되지 않는가" 3근거(수용 가능성·재사용
경로·비용/검증) 신설.)
(v0.7: QA 체계 검수(요구사항 분석 v1.1·QA 정의 v1.1) 반영 — ①
**Performance를 QA1 TTFT · QA2 throughput 2 QA로 분할**(§2.1 — 도출 체인은
공유하므로 한 절에서 함께 다룸) ② **Accuracy(신 QA3) 난이도 M→H 재판정**
(3중 품질 노출 + C-03 training-free — §2.2) ③ **Adaptability 미선정 전환**
(§2.6 삭제 → §3 미선정 표로 이동: Modifiability와 측정축 중복, 중요도
M→L) ④ 전면 재번호(번호 = 우선순위) — §4 우선순위 표 재구성.)
...
- 예: R-03 "chunk KV를 재사용하고 싶다" → 재사용이라는 **기능**(FR-02, 구 FR-04).
...
### 2.1 QA1 · QA3. Performance — throughput(decode) · TTFT(prefill) 2 QA

(v0.10 재번호: throughput = QA1 · TTFT = QA3. v0.7: 두 지표를 **독립
QA로 분할** — TTFT[목표 1] · throughput[목표 2·3]. 단일 QA 내 AND 판정은 축별 기여·DP 별점 기여를
흐린다는 검수 반영. 도출 체인(VOC→품질 문장→지표→bin)은 두 QA가 공유하므로
본 절에서 함께 다루고, 구 AND 조건은 "종합 판정에서 두 QA 모두 ★★★"
요건으로 계승한다.)
...
5. **bin(각 축 ≥ 2×)의 근거 — 두 단계를 각 단계 문헌이 독립 뒷받침**:
   ...
   **② throughput ≥ 2× (decode 축 — 목표 2·3)** (v0.8 — pruning 1차 앵커):
   [SnapKV](https://arxiv.org/abs/2404.14469)가 프롬프트 KV 92% 축소로
   16k 입력 **생성 속도 3.6×·메모리 효율 8.2×**(B),
   [H2O](https://arxiv.org/abs/2306.14048)가 KV 예산 20%로 처리량
   3×~최대 29×(B — offloading baseline 대비, 참고 상한). 조합 옵션 근거:
   [vLLM/PagedAttention (SOSP'23)](https://arxiv.org/abs/2309.06180) paging
   **처리량 2–4×**(B), [KIVI](https://arxiv.org/html/2402.02750v2) 양자화
   **처리율 2.35–3.47×**(B). pruning이 batch(용량)와 토큰당 읽기량(대역폭)
   을 동시에 줄이고 tier 확장·KV 인지 스케줄링이 이득을 시스템 처리량으로
   전환하므로 단독 기법 하단 **2×**를 요구(C).
...
1. **VOC**: R-06 (User): "압축·재사용 때문에 답변 품질이 떨어지면 쓸 수
   없다 — **상한을 보장**해달라." R-24(목표 재정의): 목표 2가 "LLM
   정확도를 유지한 채"를 목표 문장에 명시 — bound가 곧 과제 정의의 일부.
2. **품질 문장 — 왜 '개선 목표'가 아니라 'bound'인가**: 압축(양자화·토큰
   eviction)은 본질적으로 lossy다. 품질은 올리는 대상이 아니라 **지키는
   제약**이며, QA1(throughput)·QA3(TTFT)·QA4의 수치는 이 bound 안에서만
   유효하다. 이 **gate 성격**(전 수치 무효화)은 중요도 I2=2로 반영되고,
   우선순위 2는 동률군 내 난이도 최고점(6)에서 나온다 — 아무리 빨라도
   품질이 baseline과 크게 다르면 그 성능은 무효.
   *난이도 재판정(v0.7, M→H)*: 목표 재정의로 품질 영향원이 요청별 차등
   **압축(중요도 기반 토큰 pruning 주 기법, v0.8)** + **비접두
   재사용**(선택 재계산율 트레이드오프, CacheBlend 0.01–0.03(B)) +
   스케줄링의 **축출/강등 선택**의 3중이 되었고, 요청별 bound 집행은 이를
   통합 추적하는 구조를 요구하며, C-03(training-free)으로 재학습 기반 회복
   수단이 배제된다(C) — 문헌 재현(M)을 넘는 설계 부담. *v0.8 추가*: 토큰
   중요도는 쿼리 의존적(SnapKV(B))인데 재사용 KV는 미래 쿼리용 영속
   자산이라 **재사용 대상 KV의 pruning이 요청별 bound를 깨뜨릴 수 있음** —
   pruning×재사용 충돌(요구사항 v1.2 신규 쟁점, DP2×DP3)이 이 난이도의
   추가 근거.
...
### 2.3 QA4. Resource Efficiency — 유효 KV 용량 (수단, 우선순위 4)
...
3. **지표 선택 — 왜 '유효(원본 환산)' KV 용량인가**: 압축 후 저장 바이트가
   ...
   품질을 팔아 용량을 사는 눈속임이 가능하다. (pruning 주 기법에서 압축률 =
   토큰 보존률의 역수로 산입 — 공식 불변, v0.8.)
4. **왜 throughput(QA1)에 흡수되지 않는가 — 3근거 (v0.8 신설)**:
   "용량↑→batch↑→throughput↑"는 기여 경로 중 하나일 뿐, QA1은 본 QA의
   superset이 아니다 — 대역폭 포화 영역에서는 용량을 늘려도 처리량이 오르지
   않고, 스케줄링·커널 개선만으로 처리량이 올라도 메모리 병목 해소는 입증되지
   않는다(C). throughput으로 환원되지 않는 독립 가치 3건:
   ① **수용 가능성** — 장문 컨텍스트 admit 여부·보유 가능한 agent 세션 수는
   rate가 아닌 용량이 결정(저부하 interactive에서는 처리량 무관)(C)
   ② **재사용 경로** — 영속 KV 보관량 → cache hit rate → TTFT(QA3): 용량은
   QA1(throughput)만이 아니라 QA3의 상류(C)
   ③ **비용·검증** — HBM당 수용 컨텍스트가 비용 구조를 결정하고, MCAS
   시뮬레이션 예측-실측 대조(R-02)가 요구하는 수치가 정확히 이 배율(A).
5. **bin(≥3×)의 근거 — 3단 논증**:
...
### 2.4 QA5. Modifiability — KV 구조 변화·신규 tier 수용성 (framework 격리 대리 측정 포함)
...
(구 §2.5 Maintainability 체인은 v0.11에서 **미선정 전환** — §3 표로 이동.
비용 bin은 DP1 비용 모델 판단 기준으로 보존.)

(구 §2.6 Adaptability 체인은 v0.7에서 **미선정 전환** — §3 표로 이동.
framework 결합 격리는 QA5의 코어/모듈 경계 지표가 대리 측정한다.)

## 3. 미선정 6건 — 왜 이 5개"만"인가

선정 논거의 완결성은 탈락 논거에서 나온다(사유 없는 탈락은 stakeholder 배신):

| 후보 | 탈락 논거 | 처리 |
|---|---|---|
| Availability (R-18) | KV는 원본에서 **재계산 가능한 파생 데이터** — 유실은 정확성이 아닌 성능 문제로 환원(C), QA1(throughput)·QA3(TTFT) 측정에 흡수 | 실패모델은 DP3·DP5 구조 결정으로; 상용화 재평가 |
| Security (R-19) | 실증 단계 재사용 범위를 **사용자/세션 내로 한정**하면 위협 자체가 소거(정책 제약으로 완화) | DP3 커플링; 멀티테넌트 상용화 시 독립 QA로 |
| Interoperability (R-15) | 독립 QA가 아니라 **DP1의 결정 변수** — plugin/독립 선택이 곧 호환성 수준을 결정 | QA5 bin(upstream+2주)·DP1 비용 모델이 대리 측정 |
| Maintainability (R-08·R-12·R-13) — v0.11 미선정 전환 | 초기 구축 인월·유지 FTE는 **연구 과제 성패의 critical 요구가 아님** — rubric 중요도 2(성패 지표 아님)·난이도 2(표준 산정 방법)로 양축 최하(C). 비용이 갈리는 사실(plugin 수 인월 vs 독립 수십 인월+(A))은 유효하나 이는 특정 DP(플랫폼 결정)의 판단 기준이지 시스템 품질속성이 아님 | 구 bin(≤6인월·≤0.5 FTE)은 DP1 비용 모델 판단 기준으로 보존; 상용화(운영 조직 등장) 시 재평가 |
| Adaptability (R-23) — v0.7 미선정 전환 | ① 측정축(코어 변경 LOC·시그니처·전환 공수)이 **QA5와 중복** — framework 결합이 코어에 새면 QA5 지표가 즉시 악화하므로 대리 측정 성립(C) ② 1단계는 단일 framework 위 실증이 목적 — 교체 리스크 노출 시점은 2단계/상용화 → 중요도 M→L ③ 어댑터 경계는 DP1의 결정 변수 | 구 bin(결합 ≤10%·전환 ≤3인월)은 DP1 평가 참고로 보존; upstream 방향 전환 현실화 또는 2단계 진입 시 재평가 |
| Scalability (R-20) | **고정 N 테스트베드 전제**(§3.1)로 N 조정 자체가 범위 외 → 중요도 L | N 조정 진화 경로 활성화 시 재평가 |

## 4. 우선순위 산정 — 중요도·난이도 판정 근거 (v0.10 규칙 개정)

**판정 방법**: 각 축을 3개 세부 기준 × 0/1/2점으로 채점, **H ≥ 5 · M 2–4 ·
L ≤ 1** (rubric 정의·전체 채점표는 QA 정의 문서 v1.3이 단일 출처).
중요도 = I1 목표 직결성 / I2 파급 범위 / I3 VOC 대체 불가성.
난이도 = D1 신규 설계 필요성 / D2 동시 충족 폭 / D3 회복·가역성.

우선순위 규칙(v0.10 개정): **① 중요도(rubric 점수) → ② 난이도(rubric
점수) → ③ 잔여 동률은 성능 사슬 내 역할(목표 > 전제 gate > 수단).**
ATAM 표준 순서 — (중요도 H, 난이도 H)가 최상위 ASR. gate의 파급(전 수치
무효화)은 중요도 I2에 반영되므로 별도 역할 우대는 잔여 동률에만 쓴다(C).

| QA | 중요도 판정 (I1·I2·I3) | 난이도 판정 (D1·D2·D3) | 우선순위 |
|---|---|---|---|
| QA1 Throughput | **H 최고** (6 = 2·2·2) — 목표 2·3 공동 판정(미달 시 복수 목표 실패)·decode 병목 70–85%(A) 해소 판정 | **H** (5 = 2·2·1) — KV 공간 확보 선택 정책은 문헌 공백 + iso-latency×3메커니즘×ablation 동시 충족 | 1 (중요도 단독 최고) |
| QA2 Accuracy | **H** (5 = 1·2·2) — gate: 전 QA 수치 무효화(파급 최대)·R-06 전용 | **H 최고** (6 = 2·2·2) — 요청별 bound 집행·3중 추적은 문헌 공백 + training-free로 회복 수단 부재 | 2 (5점 동률군 내 난이도 최고) |
| QA3 TTFT | **H** (5 = 2·1·2) — 목표 1 단독 판정·전용 VOC(R-03·04·05) | **M 상위** (3 = 1·1·1) — 경로 2/3가 문헌 재현(CacheBlend·SGLang), 신규는 agent memory 축(복원 vs 재계산)에 국한 | 3 (QA4와 완전 동률 → 잔여 동률 역할: 목표 > 수단) |
| QA4 Resource Efficiency | **H** (5 = 2·1·2) — **목표 2 공동 판정**: 기전(병목 해소) 미입증 시 처리량이 올라도 인과 불성립 = 목표 2 실패 + R-02 대체 불가 | **M** (3 = 1·2·0) — pruning 확정으로 도달선 문헌화(H2O 20% = 5×(B)), 잔여는 3중 제약 하 운용 튜닝(반복 조정 가능) | 4 (수단·기전 — 잔여 동률 역할) |
| QA5 Modifiability | **M** (3 = 0·1·2) — 1단계 판정 비직결, 수명·2단계 접속점 | **H** (5 = 2·1·2) — MLA·linear attention 수용 경계는 문헌 무해답 + 코어/공개 IF는 불가역 초기 결정 | 5 (M그룹 유일 잔존) |

**변별력 메모**: H/H는 **QA1(throughput)·QA2(Accuracy) 2건뿐** — QA1은
중요도 최고점(6), QA2는 난이도 최고점(6)으로 축이 다르다. QA3·QA4는
중요도 H·난이도 M으로 "중요하지만 도달 경로가 문헌에 있는" 축, QA5는 그
역(M/H — 덜 급하지만 되돌리기 어려운 축)이다.

## 5. PPT 반영 가이드 (P7 슬라이드)

- 헤드라인: "품질 발화 10건 → 시나리오화 → Utility Tree(후보 11) → **QA
  5건 선정**" (§0 5단계를 소형 흐름도로. 선정 5 — 과제 표준.
...
- 미선정 6건은 슬라이드 하단 1줄씩(§3 표의 탈락 논거 열).
