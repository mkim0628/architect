...
**개정 이력**
- v1.7: **VOC·stakeholder 재정제 — 구 과제(자사 디바이스 활용) 잔재 제거
  (검수 반영).** KV 캐시 최적 운용 과제 기준으로 부록 A를 재정제:
  ① **R-09(PIM/PNM E2E 테스트) 결번 처리** — 과제 재정의(R-24) 전의 구
  과제 VOC로, 현 과제의 요구가 아니라 **2단계(MCR 완성) 과제의 VOC 등록부로
  이관** ② **R-07 재작성** — "E2E 제품 가치 확인"(구 과제 문맥)을 현
  과제에 유효한 요구로 재수집: "KV 운용 계층이 자사 메모리 확장(2단계)의
  전제가 되도록 **tier 추상화 접속점을 보존**하라" → QA-05·FR-04로 정제
  ③ 메모리 사업부 stakeholder 행을 현 과제 관심사(접속점 보존·레퍼런스
  스택·디바이스 불변 전제)로 재기술 ④ 정제 요약 수량 갱신(유효 22건,
  2단계 배정 분류 폐지). FR·QA 후보·선정·우선순위 불변
...
|---|---|---|
| 메모리 사업부 (제품기획·디바이스 설계팀) | 과제 발주 주체의 일원. 메모리 제품 로드맵 보유, MCR을 레퍼런스 SW 스택으로 요구 | 현 과제(KV 캐시 최적 운용)에서의 관심사: **KV 운용 정책 계층의 성립과 tier 추상화 접속점 보존**(R-07 — 향후 자사 디바이스 확장[2단계]의 전제), 레퍼런스 스택 확보(R-08), 디바이스 불변 전제(R-10). (구 과제 VOC였던 자사 디바이스·근접연산 E2E 실증은 2단계 과제 VOC로 이관 — R-09 결번) |
...
**구 FR의 처리**: 구 FR-07(근접연산 오프로드 — 구 과제 VOC, 현 결번 R-09)·구 FR-08(디바이스
...
2단계 이관(v1.0)으로 본 표에서 제외 — 2단계 과제의 요구사항 분석(구 R-09
이관분 포함)에서 복원한다.
...
QA-02(Accuracy) ← R-06 / QA-03(TTFT) ← R-01·R-03·R-04·R-05·R-11·R-24 / QA-04 ← R-01·R-02 / QA-05 ←
...
| R-07 | 메모리 사업부 | (v1.7 재수집 — 현 과제 기준 재진술) "KV 운용 정책 계층은 향후 **자사 메모리 확장(2단계)이 성능이 되기 위한 전제**다 — tier를 파라미터로 추상화한 **접속점을 보존**해, 디바이스가 준비되면 구조 변경 없이 편입되게 하라." (구 진술 "E2E 제품 가치 확인"은 과제 재정의 전 문맥 — 2단계 VOC로 이관) | QA-05, FR-04 |
| R-08 | 메모리 사업부 | "우리가 **자체 레퍼런스 스택**을 가지고 있으면 고객에 제공할 수 있다." | FR-01, QA-06 |
| R-09 | — | (결번 — v1.7. 구 진술 "PIM/PNM 연산의 효용성을 E2E에서 테스트하고 싶다"는 **과제 재정의(R-24) 전의 구 과제 VOC** — 현 과제(KV 캐시 최적 운용)의 요구가 아니므로 본 분석에서 제외하고 **2단계(MCR 완성) 과제의 VOC 등록부로 이관**. 설계 자산 DP6–DP8·ADR-001은 2단계 자산으로 보존, 접속점 보존 요구는 R-07(재수집)이 승계) | — |
