...
| QA2 | ★★☆ | **★★★** | 다단 백엔드(CPU DRAM·SSD·원격)+CacheGen 압축 상속 — 원본 환산 수용량 확장이 주특기(B), 자사 CXL tier는 백엔드 모듈로 즉시 기여(C) |
| QA3 | ★★★ | **★★☆** | 정책 골격이 LMCache 소유 — 요청별 품질 예산 차등 집행 통로 없음, "전역 bound만 보장" bin(C) |
...
- QA 프로파일 요약: 후보1은 QA5 최강·QA1 상한 제한(변형 B는 QA2 최강·QA3
  집행 단위 열위), 후보2는 QA1·QA2 상한 최고·QA5 최악 — 두 후보의 trade-off가
