...
| QA2 | ★★☆ (F) | CacheGen 손실 압축 + CacheBlend selective recompute로 품질 회복 메커니즘 자체는 문헌 검증(B). 그러나 정책 골격이 LMCache 소유라 요청별 품질 예산 차등 집행 통로가 없음 — "전역 bound만 보장" ★★☆ bin 정의에 부합(C) |
| QA3 | ★★★ (F) | ≥3× bin 예측: 다단 백엔드(CPU DRAM·SSD·원격) + CacheGen 압축을 상속해 원본 환산 수용량 확장이 이 후보의 주특기(B — 프로덕션 채택 사례); 자사 CXL tier는 백엔드 모듈로 즉시 기여(C). 단 재적재 지연이 가용 비율을 깎을 리스크(C) |
...
- QA 프로파일 요약: 후보3은 QA3·QA5에서 최강, QA1 상한과 QA2 집행 단위에서
