// Regenerate: NODE_PATH=<dir-with-pptxgenjs> node docs/1_과제배경/mcr_deck.build.js
...
// ───────────────── 1. 과제 배경 (navy, 3단 × 2, 이미지) ─────────────────
A.pageColumns(pptx, {
  title: "과제 배경", page: 1, band: "navy",
  cols: [
    { header: "LLM 추론 메모리 병목 심화", items: [
      { text: "Decode 단계가 추론 시간을 지배 — memory-bandwidth-bound 연산, 자체 P/D 벤치 실측 대기시간 70–85%", image: img("bg_decode.png") },
      { text: "KV cache가 컨텍스트·세션 수에 비례 증가 → HBM 용량 압박, agent는 세션 넘는 장기 기억 요구", image: img("bg_kv.png") },
    ]},
    { header: "Memory-Centric 디바이스 포트폴리오", items: [
      { text: "PIM · PNM · Custom HBM · HBF · CXL 메모리 확장 등 자사 memory-centric 제품군 확보", image: img("bg_devices.png") },
      { text: "근접연산 · 대역폭 · 용량의 서로 다른 축 → 조합 시 GPU HBM 단일 체계로 불가능한 설계 공간", image: img("bg_axes.png") },
    ]},
    { header: "문제 공간의 재정의", items: [
      { text: "연산기 중심 → 메모리 중심: '연산을 데이터가 있는 곳으로' (PIM/PNM), 배치·이동·스케줄 가정 재검토", image: img("bg_shift.png") },
      { text: "메모리 계층 심화: HBM–HBF–DRAM–CXL–SSD 5단↑ → 정적 tiering 한계, 기존 vLLM 가정 수용 불가", image: img("bg_hierarchy.png") },
    ]},
  ],
});

// ───────────────── 2. 과제 필요성 (navy, 2단 × 2, 이미지) ─────────────────
A.pageColumns(pptx, {
  title: "과제 필요성", page: 2, band: "navy",
  cols: [
    { header: "이종 메모리 1급 런타임 아키텍처 (기술 요구)", items: [
      { text: "계층화 이종 메모리·근접연산을 1급 개념으로 수용 (←배경 1.3): topology model, 정책/메커니즘 분리, 결합방식 DP화", image: img("nec_arch.png") },
      { text: "배치×압축×재사용 joint orchestration (←배경 1.1): SLO·품질 예산 기준 공동 조율, 장기 기억까지 시간축 확장", image: img("nec_orch.png") },
    ]},
    { header: "레퍼런스 스택 & E2E 증명 (사업 가치)", items: [
      { text: "자사 메모리 제품군의 레퍼런스 SW 스택 (←배경 1.2): 응용(LLM 서빙)과 디바이스를 잇는 스택", image: img("nec_stack.png") },
      { text: "GPU HBM 단일 tier 대비 성능 개선을 정량 입증하는 E2E 증명 vehicle → 설계 산출물이자 SW 사업 자산", image: img("nec_e2e.png") },
    ]},
  ],
});

// ───────────────── 3. 과제 범위 · 목적 (navy, 3단, 텍스트 전용) ─────────────────
A.pageColumns(pptx, {
  title: "과제 범위 · 목적", page: 3, band: "navy",
  cols: [
...
// ───────────────── 4. 과제 개요 (green, 2단: 표 + overall architecture) ─────────────────
A.pageOverview(pptx, {
  ...
  // architecture: img("overall_arch.png"),  // 지시가 있을 때만. 없으면 공란.
});
